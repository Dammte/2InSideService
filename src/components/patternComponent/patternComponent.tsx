...
interface PatternLockProps {
  ...
  pattern: number[]; // Nueva prop para controlar el patrón desde el padre
}
...
const PatternLock: React.FC<PatternLockProps> = ({ onPatternComplete, pattern }) => {
  ...
  const drawCanvas = () => {
    ...
    if (isDrawing && currentPos && selectedPattern.length > 0) {
      const lastDot = dotPositions[selectedPattern[selectedPattern.length - 1]];
      ctx.beginPath();
  ...
  useEffect(() => {
    setSelectedPattern(pattern); // Sincroniza con el patrón del padre
  }, [pattern]);
